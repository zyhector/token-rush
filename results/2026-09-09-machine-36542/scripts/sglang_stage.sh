...
serve() { local name=$1; shift
  ...
  SPID=$!
  for i in $(seq 1 240); do sleep 5; curl -sf http://127.0.0.1:30000/health >/dev/null 2>&1 && { echo "server $name up after $((i*5))s"; return 0; }; grep -q "Traceback\|Error" $R/server_$name.log && ! pgrep -f "sglang.launch_server" >/dev/null && { echo "server $name DIED"; tail -5 $R/server_$name.log; return 1; }; done
...
echo "===== SGLang raw + context ====="
if serve raw --context-length 262144 --chunked-prefill-size 16384 --max-prefill-tokens 16384 --mem-fraction-static 0.88; then
