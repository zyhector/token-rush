#!/bin/bash
R=/workspace/token-rush/results/2026-09-09-machine-36542/vllm; V=/workspace/venvs/vllm/bin; cd /workspace/token-rush
...
serve() { # name, args...
  ...
  SPID=$!
  for i in $(seq 1 240); do sleep 5; curl -sf http://127.0.0.1:8000/v1/models >/dev/null 2>&1 && { echo "server $name up after $((i*5))s"; return 0; }; kill -0 $SPID 2>/dev/null || { echo "server $name DIED"; tail -5 $R/server_$name.log; return 1; }; done
...
echo "===== vLLM raw + context (220k) ====="
if serve ctx --max-model-len 220000 --max-num-batched-tokens 8192 --gpu-memory-utilization 0.92; then
  $V/python scripts/rivals/vllm_bench.py raw 2>&1 | tee $R/results_raw.log
  $V/python scripts/rivals/vllm_bench.py context 0 22000 90000 200000 2>&1 | tee $R/results_context.log
fi; stop
echo "===== vLLM MTP 1 ====="
