...
| 3b (2–3 d) | **DFlash2 as the draft**: z-lab's 1.92B block-diffusion draft for this model, 7 drafts from one forward conditioned on the target's layer 5/19/33/47/61 hidden states; measured teacher-forced on our target at 3.57 / 5.37 / 5.76 accepted per step (step 24). Build its forward on the fused kernels (KV injection, in-block bidirectional mask, convs, selector), int4-pack it, verify at K=7 | Projected 238 / 358 / 384 tok/s: every effective-throughput target above |
| 3c (1–2 d) | **Marlin-class M-row int4 GEMM**: vendor the Marlin kernel (Apache-2) as a torch 2.14 / CUDA 13.3 extension for `sm_120`, repack our weights to its layout, use it for the M-row verify (and M=1 if it wins); the verify overhead is the M-row kernel's per-block work (steps 19, 23) | Verify 1.35x -> ~1.15x at K=7: ~+15% on every speculative number (~275 / 413 / 443 projected) |
