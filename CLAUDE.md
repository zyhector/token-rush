...
| 1b (1 wk) | Correctness and quality, once there is something to check: bf16 path vs HF (engine-correctness gate, per-layer hidden-state diff first, greedy divergence last; a second 5090 rented for the days it takes makes bf16 fit without offload), quantization choice at ≤4.25 bpw against the quality gate, KL table vs bf16 and vs rivals' quants through the same forward | Correctness baseline + quality table |
