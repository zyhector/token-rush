...
from tokenrush.spec import generate_spec  # noqa: E402
...
def main():
    ...
    eng = Engine(cfg, w, max_len=4096, max_spec=a.k, consistent=a.consistent)
    eng.capture()
    ...
    for name, text in PROMPTS.items():
        ...
        print(f"\n{name}: raw {st_raw['decode_tok_s']:.1f} tok/s | spec K={a.k}: {st_spec['decode_tok_s']:.1f} tok/s, "
              f"{st_spec['accepted_per_step']:.2f} tokens/step, {st_spec['ms_per_step']:.1f} ms/step (eager drafts) | "
              f"identical for {same}/{n} tokens" + ("" if same == n else "  <-- MISMATCH"))
    ...
    if a.profile:
        ...
        with profile(activities=[ProfilerActivity.CUDA]) as prof:
            eng.verify(drafts); torch.cuda.synchronize()
        rows = [(e.key, e.count, e.self_device_time_total) for e in prof.key_averages() if e.self_device_time_total > 0]
        print(f"\nK={a.k} verify step, {sum(r[2] for r in rows) / 1e3:.2f} ms GPU, top kernels:")
